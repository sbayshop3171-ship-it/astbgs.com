        ...
        @auth
            <form action="{{ route('user.author.mail', $author->id) }}" class="support-form" method="POST">
                ...
                <div class="form-group">
                    <label class="form--label required">@lang('From')</label>
                    <input type="email" class="form--control" value="{{ auth()->user()->email }}" name="email"
                        placeholder="@lang('Email')" readonly>
                </div>
                <div class="form-group">
                ...
                @endphp
                <div class="text-end">
                    <button type="submit" class="btn btn--base btn--md {{ $buttonDisabled }}"
                        {{ $buttonDisabled }}>@lang('Send')
                    </button>
                </div>
            </form>
        @else
