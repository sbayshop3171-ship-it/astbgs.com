...
class AdminController extends Controller
{
    public function dashboard()
    {
        ...
        $widget['total_products']         = (clone $productQuery)->count();
        $widget['approved_products']      = (clone $productQuery)->approved()->count();
        $widget['pending_products']       = (clone $productQuery)->pending()->count();
        $widget['soft_rejected_products'] = (clone $productQuery)->softRejected()->count();
        $widget['hard_rejected_products'] = (clone $productQuery)->hardRejected()->count();


        $widget['total_download']          = Earning::sum('download_count');
        $widget['total_download_amount']   = Earning::sum('total_earning');
        $widget['total_active_file']       = Product::approved()->count();
        $widget['total_referral_amount']  = Transaction::where('remark', 'referral_commission')->sum('amount');
        ...
        $topDownloadItems = Product::select('title', 'total_download', 'inline_preview_image', 'slug')
            ->orderByDesc('total_download')
            ->take(10)
            ->get();

        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'deposit', 'withdrawals', 'topDownloadItems'));
    }

    public function getTopDownloadingItems(Request $request)
    {
        $timePeriod      = $request->input('timePeriod', 'all');
        $topDownloadingItems = Product::select('title', 'total_download', 'inline_preview_image', 'slug', 'id')
            ->when($timePeriod !== 'all', function ($query) use ($timePeriod) {
                $startDate = match ($timePeriod) {
                    '1' => Carbon::now()->subMonth(),
                    '2'     => Carbon::now()->subMonths(6),
                    '3'     => Carbon::now()->subYear(),
                    '4'     => Carbon::now()->startOfMonth(),
                    default => null,
                };
        ...
        $content = view('admin.product.top_download_product', compact('topDownloadingItems'))->render();
        return response()->json(['content' => $content]);
    }

    public function getDownloadData(Request $request)
    {
        $timePeriod = $request->input('timePeriod', '1_month');
        ...
        $downloadData = DownloadLog::selectRaw('DATE(created_at) as date, COUNT(*) as total_downloads')
        ...
        $chartData = [
            'categories' => $downloadData->pluck('date')->toArray(),
            'series'     => [
                [
                    'name' => 'Total Downloads',
                    'data' => $downloadData->pluck('total_downloads')->map(fn($value) => (int) $value)->toArray(),
                ],
            ],
        ];
