...
class UserController extends Controller
{
    public function home()
    {
        ...
        $unRepliedComments = $author->comments()
            ->where('review_id', 0)
            ->where('parent_id', 0)
            ->whereDoesntHave('replies', fn($q) => $q->where('user_id', $author->id))
            ->count();

        $unRepliedReviews = $author->reviews()
            ->whereDoesntHave('replies', fn($q) => $q->where('user_id', $author->id))
            ->count();
    ...
    public function referredUsers(){
        $pageTitle = 'My Reffered Users';
        $users = User::active()->where('ref_by', auth()->id())->orderBy('id', 'DESC')->searchable(['firstname', 'lastname','email', 'username'])->paginate(getPaginate());
        return view('Template::user.referrals.reffered_users', compact('pageTitle', 'users'));
    }
