...
class ProductController extends Controller
{
    ...
    public function awaitingProducts()
    {
        ...
        $products = Product::whereIn('subcategory_id', $reviewer->subcategories ?? [])->where(function ($q) {
    ...
    {
        $pageTitle = 'Permanently Disabled Items';
    ...
    public function activitiesAjax($id)
    {
        if (!request()->ajax()) {
            return response()->json(['html' => '', 'hasMore' => false]);
        }

        $product = $this->productData(null, true)->find($id);
    ...
    public function rejectProduct(Request $request, $productId, $type)
    {
        ...
        if (in_array($type, [Status::PRODUCT_HARD_REJECTED, Status::PRODUCT_PERMANENT_DOWN]) && !DownloadLog::where('product_id', $product->id)->exists()) {
            $fileUploader          = new FileUploader();
            $fileUploader->path    = getFilePath('productFile') . '/' . $product->slug;
            $fileUploader->oldFile = $product->file;
            $fileUploader->removeOldFile();

            $screenshotPath = getFilePath('screenshots') . '/' . $product->slug . '/screenshots';
            if(is_dir($screenshotPath)){
                fileManager()->removeDirectory($screenshotPath);
            }
        }
    ...
    {
        $reviewer = auth('reviewer')->user();
        $products = Product::whereIn('subcategory_id', $reviewer->subcategories ?? [])->with(['category', 'subCategory'])->searchable(['title'])->filter(['is_free'])->dateFilter();
